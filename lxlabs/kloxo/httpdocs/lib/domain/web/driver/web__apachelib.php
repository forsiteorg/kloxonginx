...
var $nginx = "/usr/local/nginx/conf/";
...
static function installMe()
{
	...
	lxfile_cp("/usr/lib/httpd/modules/mod_rpaf*.so", "/etc/httpd/modules/mod_rpaf.so");
	lxfile_cp("/usr/local/lxlabs/kloxo/file/centos-5/rpaf.conf", "/etc/httpd/conf.d/rpaf.conf");
	...
	lxfile_mkdir("/home/nginx");
	lxfile_mkdir("/home/nginx/logs");
	lxfile_mkdir("/home/nginx/conf");
	lxfile_mkdir("/home/nginx/conf/domains");
	
	$ret = lxshell_return("yum", "-y", "install", "nginx");

	copy("/usr/local/lxlabs/kloxo/file/centos-5/nginx.conf", "/etc/nginx/nginx.conf");
	
	$ret = lxshell_return("/etc/init.d/nginx", "restart");
...
function createConffile()
{
	...
	$n_file = "/home/nginx/conf/domains/{$domainname}.conf";
	
	$npath = "{$this->main->getFullDocRoot()}/";

	if(!$this->main->isOn('status')) {
		if ($this->main->__var_disable_url) {
			$npath = add_http_if_not_exist($this->main->__var_disable_url);
		} else {
			$npath = "/home/kloxo/httpd/disable/";
		}
	}
...
access_log /home/nginx/logs/{$domainname}_access.log;
error_log /home/nginx/logs/{$domainname}_error.log;
...
";
	lfile_put_contents($n_file, $nstring);
	...
	for ($c = 0 ; $c < $count ; $c++) {
		...
		$string  = null;
		$string .= "<VirtualHost \\\n";
		$string .= "127.0.0.1:8080\\\n";//$this->createVirtualHostiplist("8080");
		...
		$syncto = $this->syncToPort("8080", $cust_log, $err_log);
...
static function createCpConfig()
{
	...
	$vstring = self::staticcreateVirtualHostiplist('8080');
...
static function createWebmailConfig()
{
	...
	$webdata  = null;
	$webdata .= "<VirtualHost \\\n";
	$webdata .= self::staticcreateVirtualHostiplist("8080");
